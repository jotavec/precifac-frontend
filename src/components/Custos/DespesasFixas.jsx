...
export default function DespesasFixas() {
  ...
  async function handleAddSubcat() {
    try {
      ...
      setSubcategorias(subcats => [...subcats, { ...res.data, fixedCosts: [] }]);
  ...
  async function handleSalvarSubcat(idx) {
    ...
    try {
      ...
      setSubcategorias(subcats => subcats.map((sc, i) => i === idx ? { ...sc, name: res.data.name } : sc));
    ...
    try {
      if (editandoDespesaIdx === "novo") {
        ...
        setSubcategorias(subcats => subcats.map((sc, i) =>
          i === subcatIdx ? { ...sc, fixedCosts: [...sc.fixedCosts, res.data] } : sc
        ));
      } else {
        ...
        setSubcategorias(subcats => subcats.map((sc, i) =>
          i === subcatIdx
            ? { ...sc, fixedCosts: sc.fixedCosts.map((c, j) => j === editandoDespesaIdx ? res.data : c) }
            : sc
        ));
      }
    } catch (e) { }
