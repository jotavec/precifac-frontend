...
export default function ModalRotuloNutricional({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const res = await api.get("/categorias-nutricionais");
        ...
        // opcional: alert("Erro ao carregar categorias nutricionais");
      } finally {
        setLoading(false);
      }
    })();
  ...
  // ---------- DESCRIÇÃO ----------
  async function handleAddDescricao() {
    const novaDesc = (descInput || "").trim();
    if (!novaDesc) return;
    if (
      categoriasApi.some(
        (d) => (d.descricao || "").toLowerCase() === novaDesc.toLowerCase()
      )
    )
      return;
    ...
    try {
      setBusy(true);
      // Muitos backs exigem ambos os campos no POST — mandamos unidade="" para não dar null
      const { data: cat } = await api.post("/categorias-nutricionais", {
        descricao: novaDesc,
        unidade: "",
      });
      if (!cat) return;
      ...
      setDescricoes((prev) =>
        Array.from(new Set([...(prev || []), novaDesc]))
      );
      setDescInput("");
      setDescEditIdx(null);
    } catch (e) {
  ...
  async function handleSaveEditDescricao(idx) {
    const novaDesc = (descInput || "").trim();
    ...
    try {
      setBusy(true);
      // UPDATE costuma aceitar parciais, mas para máxima compatibilidade mandamos os 2 campos
      const { data: cat } = await api.put(
        `/categorias-nutricionais/${categoria.id}`,
        { descricao: novaDesc, unidade: categoria.unidade || "" }
      );
      if (!cat) return;

      setCategoriasApi((prev) => prev.map((c) => (c.id === cat.id ? cat : c)));
      setDescricoes((prev) =>
        prev.map((d, i) => (i === idx ? novaDesc : d))
      );
      setDescEditIdx(null);
      setDescInput("");
    } catch (e) {
  ...
  async function handleDeleteDescricao(idx) {
  ...
  async function handleAddUnidade() {
    ...
    if (
      categoriasApi.some(
        (u) => (u.unidade || "").toLowerCase() === novaUnid.toLowerCase()
      )
    )
      return;
    ...
    try {
      ...
      setCategoriasApi((prev) => [...prev, cat]);
      setUnidades((prev) =>
        Array.from(new Set([...(prev || []), novaUnid]))
      );
      setUnidInput("");
      setUnidEditIdx(null);
    } catch (e) {
  ...
  async function handleSaveEditUnidade(idx) {
    const novaUnid = (unidInput || "").trim();
    ...
    try {
      ...
      const { data: cat } = await api.put(
        `/categorias-nutricionais/${categoria.id}`,
        { descricao: categoria.descricao || "", unidade: novaUnid }
      );
      if (!cat) return;

      setCategoriasApi((prev) => prev.map((c) => (c.id === cat.id ? cat : c)));
      setUnidades((prev) =>
        prev.map((u, i) => (i === idx ? novaUnid : u))
      );
      setUnidEditIdx(null);
      setUnidInput("");
    } catch (e) {
  ...
  async function handleDeleteUnidade(idx) {
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            >
              ...
              <button
                ...
                style={{
                  background:
                    "linear-gradient(90deg, #00cfff 60%, #2196f3 100%)",
                  border: "none",
                  color: "#fff",
                  borderRadius: 13,
                  width: 56,
            ...
            >
              {loading && (
                <div style={{ color: "#8fb9e7" }}>Carregando...</div>
              )}
