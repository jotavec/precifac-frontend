...
export default function MarkupIdeal({ calcularTotalFuncionarioObj }) {
  ...
  useEffect(() => {
    ...
    async function fetchDespesasFixas() {
      try {
        const res = await api.get("/despesasfixas");
  ...
  useEffect(() => {
    async function fetchEncargos() {
      try {
        const res = await api.get("/encargos-sobre-venda");
        if (res.data) {
  ...
  useEffect(() => {
    ...
    async function fetchAllAtivos() {
      ...
      await Promise.all(blocos.map(async (bloco, idx) => {
        try {
          const res = await api.get(`bloco-ativos/${bloco.id}`);
  ...
  useEffect(() => {
    async function fetchBlocoAtivos() {
      ...
      try {
        const res = await api.get(`bloco-ativos/${blocoId}`);
  ...
  async function salvarEdicaoModal() {
    ...
    try {
      await api.post(`bloco-ativos/${blocoId}`,
        { ativos: custosAtivosEdicao },
        { withCredentials: true }
      );
      await api.put(`markup-ideal/${blocoId}`,
        { totalEncargosReais },
        { withCredentials: true }
      );
      setCustosAtivosPorBloco(prev => ({
